...
function ResearchPanel({planet}: {planet: Planets}) {
    ...
    const {earthResearchUnlocked} = useSelector((state: RootState) => state.unlockedContent);
    ...
    return (
        <article
            className={`border rounded-md w-full p-4 transition-all duration-500 col-span-4 row-span-2 ${
                earthResearchUnlocked ? "opacity-1" : "opacity-0"
            }`}>
            ...
            <ul className="grid grid-cols-2 overflow-y-scroll no-scrollbar max-h-[80%]">
                {availableResearches.map((research) => {
                    ...
                    return (
                        <li className="relative" key={research.id}>
                            ...
                            <button
                                ...
                                className={`relative px-4 py-2 border flex h-full z-10 ${
